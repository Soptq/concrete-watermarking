...
class Watermark(nn.Module):
    ...
    def _update_beta(self):
        ...
        beta_schedule = np.logspace(np.log10(self.config.beta_min), np.log10(self.config.beta_max),
    ...
    def _loss_fn(self, data, secret):
        ...
        self.losses['mse_loss'] = utils.compute_reconstruction_loss(enc_inputs, enc_output, self.device,
                                                                    recon_type='yuv').mean()
        ...
        if self.train_encoder:
            ...
            return beta_quality * (self.losses['mse_loss'] + self.losses['lpips_loss'] + \
                                   self.losses['discriminator_loss'] + self.losses['ffl_loss']) + self.losses[
                'bce_loss']
        else:
    ...
    def load_model(self, ckpt_path, strict=True, map_location=None):
        state_dict = torch.load(ckpt_path, map_location=map_location)
        logger.info(f"Loading model from epoch:{state_dict['cur_epoch']}")
        self.encoder.load_state_dict(state_dict['encoder_state_dict'], strict=strict)
        self.encoder.train()
        self.decoder.load_state_dict(state_dict['decoder_state_dict'], strict=strict)
        self.decoder.train()
        self.discriminator.load_state_dict(state_dict['discriminator_state_dict'], strict=strict)
